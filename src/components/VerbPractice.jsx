...
const VerbPractice = ({ onComplete }) => {
 ...
 <span className="text-2xl font-bold text-primary">{item['Past Participle']}</span>
 </div>
 </div>
 ))}
 ...
 <h1 className="text-4xl md:text-5xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-br from-primary via-primary/80 to-accent mb-3">Verbs Practice</h1>
 ...
 Remember to include"sein"or"haben"if practicing perfect constructions. But here, just type the participle itself (e.g."gegangen").
 </p>
 </div>
 )}
 </div>
 ...
 className={`p-4 rounded-[1.25rem] flex flex-col md:flex-row items-center justify-between gap-3 shadow-sm border ${isCorrect ? 'bg-green-100 border-green-500/20' : 'bg-red-100 border-red-500/20'}`}
 >
 ...
 <span className="text-red-700/80 font-bold uppercase tracking-wider text-[10px]">Correct answer:</span>
 <div className="flex items-center gap-2">
 <strong className="text-red-900 font-sans text-lg tracking-tight">{currentVerb['Past Participle']}</strong>
 <button
 onClick={(e) => { e.preventDefault(); playPronunciation(currentVerb['Past Participle']); }}
 className="p-1.5 rounded-full text-red-700 hover:bg-red-200 :bg-red-800 transition-colors shadow-sm bg-white/50"
 title="Play pronunciation"
 >
